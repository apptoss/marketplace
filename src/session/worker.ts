 ...
 */
export class NewTransactionWorker {
	 ...
	 * When the set is not empty, the worker will regularly check for the account's current sequence number
	 ...
	 * This allows the worker's thread to sleep until there are pending responses to check for execution
	 ...
	 */
	private async getCurrentSequenceNumber() {
	...
	private async getNextSequenceNumber(): Promise<bigint> {
		...
		const failedSequenceNumber = this.failedSequenceNumbers.shift()
		...
		if (this.currSequenceNumberPromise === undefined) {
			this.currSequenceNumberPromise = this.getCurrentSequenceNumber()
			return this.currSequenceNumberPromise
		}

		// Increment the internal iterator
		this.currSequenceNumberPromise = this.currSequenceNumberPromise.then(
	 ...
	 * This can be overridden by subclasses to provide custom logic.
	 */
	protected async signAndSubmitTransaction(
		transaction: AnyRawTransaction,
	): Promise<string> {
		const pendingResponse =
			await this.aptosClient.transaction.signAndSubmitTransaction({
				signer: this.account,
				transaction,
			})
	...
	async start() {
		...
		while (true) {
			...
			while (
				...
				(this.isRunning || this.pendingSubmissions > 0)
			) {
				await Promise.race([
					this.newPendingResponseSignal.wait(),
			...
			const sequenceNumber = await this.getCurrentSequenceNumber()
			...
			for (const response of this.pendingResponses) {
				if (sequenceNumber > response.sequenceNumber) {
					...
					Math.floor(Date.now() / 1000) > response.expirationTimestamp
				) {
			...
			if (this.isRunning) {
				while (
					...
					this.totalPendingResponses < this.maxPendingResponses
				) {
	 ...
	 * All pending submissions and responses will be processed before the worker's thread joins.
	 */
	stop() {
		if (!this.isRunning) {
