...
async function publish() {
  ...
  move
    ...
    .then(({objectAddress}) => {
      ...
      // Regular expression to match the VITE_PACKAGE_ADDRESS variable
      const regex = /^VITE_PACKAGE_ADDRESS=.*$/m;
      const newEntry = `VITE_PACKAGE_ADDRESS=${objectAddress}`;

      // Check if VITE_PACKAGE_ADDRESS is already defined
